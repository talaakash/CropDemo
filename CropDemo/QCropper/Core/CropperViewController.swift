...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    open lazy var bottomView: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: self.view.height - 130, width: self.view.width, height: 130))
...
//        topBar.rotateButton.addTarget(self, action: #selector(rotateButtonPressed(_:)), for: .touchUpInside)
    ...
    public lazy var aspectRatioPicker: AspectRatioPicker = {
        let picker = AspectRatioPicker(frame: CGRect(x: 0, y: 0, width: view.width, height: 40))
...
// MARK: - Private Methods
    ...
    open func resetToDefaultLayout() {
        ...
        // total bottom view height set
        bottomView.size = CGSize(width: view.width, height: toolbar.height + angleRuler.height + aspectRatioPicker.height + margin)
        bottomView.bottom = view.height
        toolbar.bottom = bottomView.height
        aspectRatioPicker.bottom = toolbar.top - halfMargin
        angleRuler.bottom = aspectRatioPicker.top - halfMargin
        ...
        cropRegionInsets = UIEdgeInsets(
            top: cropContentInset.top + topHeight,
        ...
        maxCropRegion = CGRect(
            x: cropRegionInsets.left,
        ...
        scrollViewContainer.frame = CGRect(
            ...
            y: topHeight,
            width: view.width,
            height: view.height - topHeight - bottomHeight)
        ...
        scrollView.bounds = CGRect(
            x: 0,
            y: 0,
            width: defaultCropBoxSize.width,
            height: defaultCropBoxSize.height)
        ...
        overlay.cropBoxFrame = CGRect(
            center: defaultCropBoxCenter,
            size: defaultCropBoxSize)
    ...
    public func matchScrollViewAndCropView(
        animated: Bool = false,
        ...
        let center = CGPoint(
            x: zoomRect.origin.x + zoomRect.size.width / 2,
            y: zoomRect.origin.y + zoomRect.size.height / 2)
        let normalizedCenter = CGPoint(
            x: center.x / (imageView.width / scrollView.zoomScale),
        ...
        UIView.animate(withDuration: animated ? 0.25 : 0, animations: {
            ...
            let contentOffset = CGPoint(
                x: normalizedCenter.x * self.imageView.width - self.scrollView.bounds.size.width * 0.5,
